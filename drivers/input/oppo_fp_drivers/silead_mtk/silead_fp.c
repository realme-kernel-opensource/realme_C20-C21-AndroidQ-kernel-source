...
static int egistec_regulator(void)
{
    ...
    if (buck ==NULL) {
		printk("egitec enter probe regulator_get r \n");
		return -1;
    }
	printk("egitec enter probe regulator_get! \n");
    ...
    if (ldo_vout_status < 0) {
		printk("egitec enter cdfinger probe regulator_enable fail \n");
		return -1;
    }
	printk("egistec enter cdfinger probe regulator_enable success \n");
...
int silfp_dev_init(void)
{
    ...
    LOG_MSG_DEBUG(ERR_LOG,"%s, liangliang Get FP_ID from GPIO_PIN is / FP_ID = %d.  sim_gpio = %d\n", __func__,sim_val, sim_gpio);

#ifdef CONFIG_MACH_MT6765
    sim_val = fingerprint_id0;
#endif
    if(0 == sim_val){
	    LOG_MSG_DEBUG(ERR_LOG,"%s, Don't  Need to register egistec FP driver fingerprint_id0 = %d\n", __func__, fingerprint_id0);
    } else {
...
#ifdef CONFIG_MACH_MT6771
	printk("egis512_init egistec_regulator\n");
	status = egistec_regulator();
	if (!status){
		LOG_MSG_DEBUG(ERR_LOG,"%s successful egistec_regulator\n", __func__);
	} else{
		LOG_MSG_DEBUG(ERR_LOG,"%s egitec regulator_setup fail \n",__func__);
		return -1;
	}
#endif
