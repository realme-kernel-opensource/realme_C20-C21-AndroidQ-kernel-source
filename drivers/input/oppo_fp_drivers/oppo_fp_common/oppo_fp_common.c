...
#if CONFIG_OPPO_FINGERPRINT_PLATFORM == 6763 || CONFIG_OPPO_FINGERPRINT_PLATFORM == 6771 || CONFIG_OPPO_FINGERPRINT_PLATFORM == 6779 || CONFIG_OPPO_FINGERPRINT_PLATFORM == 6785 || CONFIG_OPPO_FINGERPRINT_PLATFORM == 6768 || CONFIG_OPPO_FINGERPRINT_PLATFORM == 6765
...
#define ENGINEER_MENU_GOODIX_3688   "-1,-1"
...
#define ENGINEER_MENU_SILEAD_6135   "-1,-1"
...
int fingerprint_id0 = 0;
EXPORT_SYMBOL(fingerprint_id0);
#endif
...
fp_module_config_t fp_module_config_list[] = {
    {{1, 0, -1},  FP_GOODIX_3688,  CHIP_GOODIX,  ENGINEER_MENU_GOODIX_3688},
	{{0, 1, -1},  FP_SILEAD_6135,	CHIP_SILEAD,  ENGINEER_MENU_SILEAD_6135},
	{{0, 0, -1},  FP_SILEAD_6150,	CHIP_SILEAD,  ENGINEER_MENU_SILEAD_6150},
};
#else
fp_module_config_t fp_module_config_list[] = {
...
#if CONFIG_OPPO_FINGERPRINT_PLATFORM == 6785  || CONFIG_OPPO_FINGERPRINT_PLATFORM == 6765
		if (fp_data->pdev) {
    ...
    for (i = 0; i < sizeof(fp_module_config_list)/sizeof(fp_module_config_t); ++i) {
        if ((fp_module_config_list[i].gpio_id_config_list[0] == fp_id1) &&
            ...
            switch (fp_module_config_list[i].fp_vendor_chip) {
                ...
                case FP_SILEAD_6135:
                    strncpy(fp_manu, CHIP_SILEAD, FP_ID_MAX_LENGTH - FP_ID_SUFFIX_MAX_LENGTH);
                    strncat(fp_manu, "_6135", FP_ID_SUFFIX_MAX_LENGTH - 1);
                    strncpy(g_engineermode_menu_config, fp_module_config_list[i].engineermode_menu_config, ENGINEER_MENU_SELECT_MAXLENTH - 1);
                    return FP_SILEAD_6135;
                case FP_GOODIX_3688:
                    strncpy(fp_manu, CHIP_GOODIX, FP_ID_MAX_LENGTH - FP_ID_SUFFIX_MAX_LENGTH);
                    strncat(fp_manu, "_3688", FP_ID_SUFFIX_MAX_LENGTH - 1);
                    strncpy(g_engineermode_menu_config, fp_module_config_list[i].engineermode_menu_config, ENGINEER_MENU_SELECT_MAXLENTH - 1);
                    return FP_GOODIX_3688;
                default:
...
#if CONFIG_OPPO_FINGERPRINT_PLATFORM == 6785 || CONFIG_OPPO_FINGERPRINT_PLATFORM == 6768 || CONFIG_OPPO_FINGERPRINT_PLATFORM == 6765
...
#if CONFIG_OPPO_FINGERPRINT_PLATFORM == 6765
    fingerprint_id0 = fp_data->fp_id0;

    node = of_find_compatible_node(NULL, NULL, "oppo,fp_common");
	fp_id1_gpio = of_get_named_gpio(node, "oppo,fp-id1", 0);
	fp_id1_val = __gpio_get_value(fp_id1_gpio);
    dev_err(fp_data->dev,"%s, liangliang Get FP_ID from GPIO_PIN is / FP_ID = %d.  fp_id1_gpio = %d\n", __func__,fp_id1_val, fp_id1_gpio);
